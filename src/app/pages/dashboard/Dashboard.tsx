...
export const Dashboard:React.FC<IDashboard> = ({openMenuInitial, setOpen}) => {
  ...
  return (
    <div style={{position:'relative'}}>
      <Box onClick={() => setOpen(!openMenuInitial)} style={{position:'absolute', transition:'left 300ms', zIndex:'3000', left:openMenuInitial ? '129px' : '90px'}}>{openMenuInitial ? <KeyboardArrowLeftIcon /> : <MenuIcon/>}</Box>
      <Drawer 
        variant='persistent' 
        anchor="left"
